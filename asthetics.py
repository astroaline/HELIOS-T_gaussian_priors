...
parameter_labels = {"T": r"$T$", "log_xh2o": r"$log(X_{\rm H_2O})$", "log_xhcn": r"$log(X_{\rm HCN})$", "log_xnh3":
                    ...
                    "a": r"$a$", "log_r_c": r"$log(r_c)$", "log_p_cia": r"$log(P_{\rm CIA})$", "Rstar": r"$R_\star$", "G": r"$g$"}      # labels for all possible parameters
...
parameter_colors = {"T": ['Reds',0.4], "log_xh2o": ['Blues',0.4], "log_xhcn": ['Oranges',0.4], "log_xnh3": ['Greens',0.4],
                    ...
                    "b": ["Blues", 0.7], "log_p_cia": ["YlOrBr", 0.4], "Rstar": ["YlOrRd",0.4], "G": ["YlGn", 0.4]}      # colours for plots
